...
class ConvNeXt(nn.Module):
    ...
    """
    def __init__(self, in_chans: int = 1, num_classes: int = 1, depths: list = None,
                 dims: list = None, drop_path_rate: float = 0., layer_scale_init_value: float = 1e-6,
                 head_init_scale: float = 1.):
        super().__init__()
        self.downsample_layers = nn.ModuleList()  # stem and 3 intermediate downsampling conv layers
        stem = nn.Sequential(nn.Conv3d(in_chans, dims[0], kernel_size=4, stride=4),
...
def convnext_tiny(num_classes: int = 1):
...
def convnext_base(num_classes: int = 1):
